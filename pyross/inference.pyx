...
cdef class SppSparse(SIR_type):
    """User-defined epidemic model.

    To initialise the SppSparse model,
    ...
    cdef:
        ...
        readonly np.ndarray model_parameters
        readonly pyross.deterministic.SppSparse det_model
        readonly dict model_spec
        readonly dict param_dict
        readonly list model_param_keys
        readonly object parameter_mapping
    ...
    def __init__(self, model_spec, contactMatrixt, threshold, parameters, M, fi, Omega=1, steps=4,
                                    det_method='LSODA', lyapunov_method='LSODA', rtol_det=1e-3, rtol_lyapunov=1e-3, parameter_mapping=None, time_dep_param_mapping=None):
        if parameter_mapping is not None and time_dep_param_mapping is not None:
            raise Exception('Specify either parameter_mapping or time_dep_param_mapping')
        self.parameter_mapping = parameter_mapping
        ...
        if self.parameter_mapping is not None:
            parameters = self.parameter_mapping(parameters)
        if self.time_dep_param_mapping is not None:
            self.det_model = pyross.deterministic.SppSparse(model_spec, contactMatrixt, threshold, parameters, M, fi*Omega, time_dep_param_mapping=time_dep_param_mapping)
        else:
            self.det_model = pyross.deterministic.SppSparse(model_spec, contactMatrixt, threshold, parameters, M, fi*Omega)
    ...
    def construct_A_spp_sparse(self, x):
        """construct SppSparse A. x is a sympy matrix"""
        M=self.M
        nClass=self.nClass
    ...
    def construct_B_spp_sparse(self, x):
        """constructs SppSparse B. x is a sympy array"""
        Omega=self.Omega
        M=self.M
        nClass=self.nClass
        constant_terms=self.constant_terms
    ...
    def construct_J_spp_sparse(self, x):
        """constructs SppSparse J. x is a sympy matrix"""
        M=self.M
        nClass=self.nClass
        constant_terms=self.constant_terms
        linear_terms=self.linear_terms
    ...
    def construct_fullcov_elem(self, xi, xf, dt=1): ##
        ...
        Bi = sympy.Matrix(self.construct_B_spp_sparse(xi))
        Bf = sympy.Matrix(self.construct_B_spp_sparse(xf))
        Uf = sympy.eye(self.dim) + dt * self.construct_J_spp_sparse(xf) ## tangent space approx
    ...
    def dinvcovelemd(self, p, return_x0_deriv=False, keys=None, dt=1): ##
        ...
        j = sympy.Matrix(self.construct_J_spp_sparse(xf))
    ...
    def dAd(self, p, return_x0_deriv=False, keys=None, x=None):
        """
        constructs SppSparse B. param is a string or sympy symbol. Most likely you'll wish to use 'all' string
        ...
        if x == None:
            x =sympy.Matrix( sympy.symarray('x', (nClass, M)))
        no_inferred_params = np.sum(keys)
        A=self.construct_A_spp_sparse(x)
    ...
    def dBd(self, p, return_x0_deriv=False, keys=None, x=None):
        ...
        if x == None:
            x =sympy.Matrix( sympy.symarray('x', (nClass, M)))
        no_inferred_params = np.sum(keys)
        B=self.construct_B_spp_sparse(x)
    ...
    def dJd(self, p, return_x0_deriv=False, keys=None, x=None):
        """
        constructs SppSparse J. param is a string or sympy symbol. Most likely you'll wish to use 'all' string
        keys can be passed as a integer 0,1 numpy array which selects the parameters to be used for FIM calculation
        p is a sympy array which contains epi parameters. nParams*M due to age dependence
        ...
        if x == None:
            x =sympy.Matrix( sympy.symarray('x', (nClass, M)))
        no_inferred_params = np.sum(keys)
        J=self.construct_J_spp_sparse(x)
