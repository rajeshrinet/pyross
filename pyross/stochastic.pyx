...
cdef class SIR(stochastic_integration):
    ...
    cdef rate_matrix(self, rp, tt):
        cdef:
            ...
            double alpha=self.alpha, beta=self.beta, gIa=self.gIa, lmda, rateS
        ...
        for i in range(M): #, nogil=False):
            ...
            for j in range(M): #, nogil=False):
                 rateS += beta*(CM[i,j]*Ia[j]+fsa*CM[i,j]*Is[j])/Ni[j]
            lmda = rateS*S[i]
            #
            RM[i+M,i] = alpha *lmda + FM[i] # rate S -> Ia
            RM[i+2*M,i] = alphab *lmda # rate S -> Is
...
cdef class SIkR(stochastic_integration):
    ...
    cdef rate_matrix(self, rp, tt):
        cdef:
            ...
            double beta=self.beta, lmda, rateS
            long [:] S    = rp[0  :M]
            long [:] I    = rp[M  :(kk+1)*M]
            double [:] gI = self.gI
            double [:] Ni   = self.Ni
        ...
        for i in range(M): #, nogil=False):
            ...
            for jj in range(kk):
                for j in range(M):
                    rateS += beta*(CM[i,j]*I[j+jj*M])/Ni[j]
            lmda = rateS*S[i]
            #
            RM[i+M,i] =  lmda + FM[i] # rate S -> I1
            for j in range(kk-1):
...
cdef class SEIR(stochastic_integration):
    ...
    cdef rate_matrix(self, rp, tt):
        ...
        for i in range(M): #, nogil=False):
            rateS=0
            for j in range(M):
                 rateS += beta*CM[i,j]*(Ia[j]+fsa*Is[j])/Ni[j]
            lmda = rateS*S[i]
            #
            RM[i+M  , i]     =  lmda + FM[i] # rate S -> E
...
cdef class SEI5R(stochastic_integration):
    ...
    * Im: Mortality
    ...
    cdef rate_matrix(self, rp, tt):
        cdef:
            int N=self.N, M=self.M, i, j
            double alpha=self.alpha, beta=self.beta, lmda, rateS
        ...
        for i in range(M):
            rateS=0
            for j in range(M):
                rateS += beta*CM[i,j]*(Ia[j]+fsa*Is[j]+fh*Ih[j])/Ni[j]
            lmda = rateS*S[i]
            #
    ...
    * Im: Mortality
    ...
    cdef rate_matrix(self, rp, tt):
        ...
        for i in range(M):
            rateS=0
            for j in range(M):
                rateS += beta*CM[i,j]*(A[j] + Ia[j]+fsa*Is[j]+fh*Ih[j])/Ni[j]
            lmda = rateS*S[i]
            #
            # rates from S
            RM[i,i] = sa[i] # birth rate (note also associated hard-coded increase
...
cdef class SEAIRQ(stochastic_integration):
    ...
    cdef rate_matrix(self, rp, tt):
        ...
        for i in range(M):
            rateS=0
            for j in range(M):
                rateS += beta*CM[i,j]*(A[j]+Ia[j]+fsa*Is[j])/Ni[j]
            lmda = rateS*S[i]
            # rates away from S
            RM[i+M  , i]     = lmda  # rate S -> E
